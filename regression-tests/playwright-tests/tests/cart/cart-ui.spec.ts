...
import { saveScreenshot } from '../common/helpers';

test.describe('@pr-1 장바구니 UI 테스트', () => {
  test('@regression 장바구니 전체 플로우 - 로그인부터 장바구니 확인까지', async ({ page }) => {
    // 1. 로그인 페이지 접속
    ...
    // 3. 장바구니 페이지 접속
    await page.goto(`${API_BASE_URL}/cart`);
    await page.waitForLoadState('networkidle');
    ...
    // 4. 상품 목록 페이지 접속
    await page.goto(`${API_BASE_URL}/shop`);
    await page.waitForLoadState('networkidle');
    ...
    // 테스트 종료 전 3초 대기 (브라우저 확인용)
    await page.waitForTimeout(3000);
  });

  test('@smoke @regression 로그아웃 상태에서 장바구니 접근 테스트', async ({ page }) => {
    ...
    if (isLoginPage || hasErrorMessage) {
      ...
      // 스크린샷
      await page.screenshot({
        path: 'test-results/cart-access-denied.png',
        fullPage: true
      });
      console.log('장바구니 접근 거부 스크린샷 저장');
    }
